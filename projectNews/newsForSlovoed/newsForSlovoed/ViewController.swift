...
class ViewController: UIViewController, UITextViewDelegate {
    override func loadView() {
        ...
        guard let data =  SldNewsArticle.decode() else { print("no news"); return} //обработать
    ...
    private func makeNews(date: String, isRead: Bool, message: String) -> NewsSection.SectionType {
        ...
        print("value = \(isRead)")
        return NewsSection.SectionType.article(date: textField, message: textView)
    }
...
fileprivate struct NewsSection {
    ...
    enum SectionType {
        case work
        case article(date: UITextField, message: UITextView)
    }
}
//может надо убрать
...
struct SldNewsArticle {
    ...
    static func encode(newsArticle: [SldNewsArticle]) {
        ...
        NSKeyedArchiver.archiveRootObject(classObject, toFile: HelperClass.path(key: "file.archive1"))
    }
    
    static func decode() -> [SldNewsArticle]? {
        let classObject = NSKeyedUnarchiver.unarchiveObject(withFile: HelperClass.path(key: "file.archive1")) as? HelperClass
...
extension SldNewsArticle {
    @objc(SldKitCore11NewsArticle11HelperClass)class HelperClass: NSObject, NSCoding {
        ...
        class func path(key: String) -> String {
...
func parseData(data: String) -> [SldNewsArticle] {
    ...
    for item in items {
        ...
        let date = findText(with: "(?s)(?<=date=\")(.*?)(?=\")", for: item)
        ...
        if !localeForMessage.isEmpty {
            if let id = id.first, let date = date.first {
                let dateFormatter = DateFormatter()
                dateFormatter.dateFormat = "yyyy-MM-dd"
                dateFormatter.timeZone = TimeZone(abbreviation: "GMT")
                if let calculatedDate = dateFormatter.date(from: date) {
                    newsArticle.append(SldNewsArticle(identificator: id, date: calculatedDate, message: localeForMessage, isRead: true))
                }
            }
        }
    }
...
func printData(){
    let getArticle = SldNewsArticle.decode()
