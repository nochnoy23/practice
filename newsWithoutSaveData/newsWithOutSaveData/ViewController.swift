...
extension ViewController: UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.createCellNews(message: newsStruct[indexPath.section].message["en"]!,
                            isRead: newsStruct[indexPath.section].isRead,
                            font: self.font)
        return cell
    }
...
func parseData(data: String) -> [SldNewsArticle] {
    ...
    let mutableData = replaceTag(for: "title", on: "h4", in: data)
    let items = findText(with: "(?s)(?=<item)(.*?)(/item>)", for: mutableData)
    for item in items {
